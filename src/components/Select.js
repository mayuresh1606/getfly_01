...
Select.defaultProps = {
  ...
  outerClassName: 'mb-2',
  isClearable: true,
  value: '',import React, { memo } from 'react';
  import PropTypes from 'prop-types';
  import ReactSelect from "react-select";
    ...
    return (
      <div className={props.outerClassName}>
        <ReactSelect
          ...
          onChange={e => changeHandler(e, props)}
        />
        {props.errorMsg && <span style={{color:'red'}} className="text-danger">{props.errorMsg === true ? `Please select ${props.title}.` : props.errorMsg}</span>}
      </div>
    )
  }
